...
class HaloAPI {
    ...
    while (true) {
      ...
      for (const it of items) {
        out.push({
          ...
          // Counted on the Workgroup tenant across all 327 items:
          //   supplier_part_code         259/327  (manufacturer SKU)
          //   default_supplier_part_code 212/327  (supplier/Leader stock code)
          //   qbosku                       1/327  (effectively unused here)
          // Reading qbosku alone made every product unsearchable by code.
          // Both populated codes are carried because the same item is known by
          // either — "AT-USB-PWR-2" and "CBAT-USB-PWR-2" are one product.
          sku: it.supplier_part_code || it.qbosku || '',
          altSku: it.default_supplier_part_code || ''
        });
      }
  ...
  async getItemInstances(itemId) {
    ...
    return {
      ...
      labels,
      // The barcode this product would carry regardless of how many are on the
      // shelf. `labels` is one entry per unit in stock, so it is empty for
      // anything out of stock or still on order — which is exactly when a shelf
      // label is most wanted. The frontend falls back to this so a count can be
      // typed in and printed.
      defaultLabel: this.pickNonSerialBarcode(itemId, sku, item.supplier_part_code)
    };
  }
